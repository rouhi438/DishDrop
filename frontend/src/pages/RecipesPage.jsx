...
export default function RecipesPage() {
  ...
  const { user } = useAuth();
  const { recipes, loading, error, remove, refetch, submitRating } =
    useRecipes();
  ...
  const filteredRecipes = useMemo(
    () =>
      recipes.filter((recipe) => {
        ...
        return (
          (!selectedCuisine || cuisine === selectedCuisine.toLowerCase()) &&
  ...
  return (
    <main id="recipes">
      <section className="recipes-intro">
        ...
        <p>
          Browse real recipes, filter by cuisine, and save your vote for the
      ...
      <section className="recipes-shell" aria-label="Recipe collection">
        ...
        {loading && (
          <div className="recipes-state" role="status">
            <span className="loader" />
            Waking up the kitchen…
          </div>
        )}
        {!loading && error && (
          <div className="recipes-state error-state" role="alert">
        ...
        {!loading && !error && paginatedRecipes.length === 0 && (
          <div className="recipes-state">
            ...
            <p>
              Try a different filter or add the first recipe in this category.
            </p>
          </div>
        )}
        {!loading && !error && (
          <RecipeList
            ...
            onView={(recipe) => setSelectedRecipeId(recipe.id)}
        ...
        {totalPages > 1 && (
          <nav className="pagination-wrapper" aria-label="Recipe pages">
            <button
