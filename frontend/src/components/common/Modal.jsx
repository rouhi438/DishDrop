      ...
      >
        ...
        <div className="modal-content">
          ...
          <section className="user-rating" aria-label="Rate this recipe">
            <div>
              <strong>
                {count ? `${average.toFixed(1)} / 5` : "Not rated yet"}
              </strong>
              <span>
                {count
            ...
            <div className="rating-buttons">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={
                    star <= Number(recipe.userRating || 0) ? "selected" : ""
                  }
                  onClick={() => handleRate(star)}
                  aria-label={`${star} star${star === 1 ? "" : "s"}`}
                  aria-pressed={star === recipe.userRating}
                >
          ...
          <section className="recipe-copy">
            <h3>Ingredients</h3>
            <p dir="auto">{recipe.ingredients}</p>
            <h3>Instructions</h3>
            <p dir="auto">{recipe.instructions}</p>
          </section>
          {isOwner && (
